// Declare global Foundry objects to avoid TypeScript errors
// @ts-ignore
...
function isSequencerActive(): boolean {
    return typeof (window as any).Sequencer !== "undefined" && game.modules.get("sequencer")?.active;
}
function isJB2AActive(): boolean {
...
Hooks.on('createChatMessage', async (msg: ChatMessage) => {
    ...
    if (game.user?.targets.size) {
        targetToken = Array.from(game.user.targets)[0] as Token;
    }
    // Door targeting (placeholder)
    let doorTarget: DoorControl | undefined;
...
(window as any).playActionAnimation = function({
    type,
    sourceTokenId,
    targetTokenId,
    rollResult
...
function isWeatherControlActive(): boolean {
...
Hooks.once('ready', async () => {
    if (isWeatherControlActive()) {
        ...
        Hooks.on('dnd5e.preRollAttack', (item: AnyObject, config: AnyObject, options: AnyObject) => {
        ...
        (window as any).renderTemplate("modules/foundryvtt-module/templates/effect.html", overlayData).then((html: string) => {
...
Hooks.on('diceSoNiceRollComplete', (message: AnyObject) => {
...
Hooks.on('updateActor', (actor: AnyObject, changes: AnyObject, options: AnyObject, userId: AnyObject) => {
    ...
    if (changes.data?.attributes?.hp?.value !== undefined) {
        ...
        if (hp < 5) mood = 'desperate';
        else if (hp < 15) mood = 'angry';
        else mood = 'confident';
...
Hooks.on('dnd5e.useItem', async (item: AnyObject, config: AnyObject, options: AnyObject) => {
    if (!isSequencerActive() || !isJB2AActive()) return;
    if (item.type === 'spell') {
